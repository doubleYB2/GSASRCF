function [g_f,h_f]=ADMM_solve_h1(params,use_sz,model_xf,yf,small_filter_sz,w,model_w,frame,M)
...
%     ws=bsxfun(@times,w,model_w);
    ws=w;
    T = prod(use_sz);
    S_xx = sum(conj(model_xf) .*M .* conj(M) .* model_xf, 3);
    ...
    while (i <= params.admm_iterations)
        ...
        g_f = 1/(T*mu) * bsxfun(@times, yf, model_xf.* M .* conj(M)) - (1/mu) * l_f + h_f - ...
            (bsxfun(@rdivide,(1/(T*mu) * bsxfun(@times, model_xf, (S_xx .* M .* conj(M) .* yf)) - (1/mu) * bsxfun(@times, model_xf, S_lx) + bsxfun(@times, model_xf, S_hx)), B));
